...
export const sendSignUpEmail = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    - Inestment goals: ${event.data.investmentGoals}
    ...
    await step.run("send-welcome-email", async () => {
      ...
      const introText =
        ...
        "Thanks for joining AuraLyst. You now have the tools to track markets and make smarter moves !";
...
export const sendDailyNewsSummary = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    if (!users || users.length === 0)
      return { success: false, message: "No uses for news email" };
