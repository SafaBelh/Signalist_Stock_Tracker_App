...
export async function getWatchlistData(symbols: string[]) {
  if (!FINNHUB_TOKEN) throw new Error("FINNHUB API key missing");
  if (!symbols?.length) return [];

  return Promise.all(
    symbols.map(async (symbol) => {
      try {
        // 1️⃣ Quote: price, change
        const quoteRes = await fetch(
          `${FINNHUB_BASE_URL}/quote?symbol=${encodeURIComponent(symbol)}&token=${FINNHUB_TOKEN}`
        );
        const quote = await quoteRes.json();

        // 2️⃣ Profile: name, market cap, shares
        ...
        return {
          ...
          peRatio:
            profile?.shareOutstanding && price
              ? (marketCap / (profile.shareOutstanding * price)).toFixed(2)
              : "-",
        };
      } catch (err) {
        console.error("Error fetching symbol data for", symbol, err);
        return {
          symbol,
          company: symbol,
          priceFormatted: "-",
